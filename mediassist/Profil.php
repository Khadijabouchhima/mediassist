...
$query = "
    SELECT 
        u.*, 
        up.profile_picture, up.date_of_birth, up.weight, up.height, up.gender, 
        up.blood_type, up.phone_number, up.address 
    FROM users u
...
// Allowed blood types enum values
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_profile'])) {
    ...
    try {
        ...
        $blood_type_input = $_POST['blood_type'] ?? $user['blood_type'];
        // Validate blood type
        $blood_type = in_array($blood_type_input, $allowed_blood_types) ? $blood_type_input : null;
        ...
        mysqli_stmt_execute($stmt);
        $result = mysqli_stmt_get_result($stmt);
        $profile_exists = mysqli_num_rows($result) > 0;

        if ($profile_exists) {
            // Update existing profile
            $query = "UPDATE user_profiles SET date_of_birth = ?, weight = ?, height = ?, gender = ?, blood_type = ?, phone_number = ?, address = ? WHERE user_id = ?";
            $stmt = mysqli_prepare($connection, $query);
            ...
            if (!$update_success) {
                throw new Exception("User profile update failed: " . mysqli_error($connection));
            }
        } else {
            // Insert new profile
            $query = "INSERT INTO user_profiles (user_id, date_of_birth, weight, height, gender, blood_type, phone_number, address) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
            $stmt = mysqli_prepare($connection, $query);
            mysqli_stmt_bind_param($stmt, "issdssss", $user_id, $date_of_birth, $weight, $height, $gender, $blood_type, $phone_number, $address);
            $insert_success = mysqli_stmt_execute($stmt);

            if (!$insert_success) {
                throw new Exception("User profile insert failed: " . mysqli_error($connection));
            }
        }
        ...
        if (isset($_FILES['profile_pic']) && $_FILES['profile_pic']['error'] === UPLOAD_ERR_OK) {
            ...
            if (move_uploaded_file($_FILES['profile_pic']['tmp_name'], $filePath)) {
                // Check if user_profiles row exists for profile picture update
                $query = "SELECT id FROM user_profiles WHERE user_id = ?";
                $stmt = mysqli_prepare($connection, $query);
                mysqli_stmt_bind_param($stmt, "i", $user_id);
                mysqli_stmt_execute($stmt);
                ...
                if ($profile_exists) {
                    ...
                    $query = "UPDATE user_profiles SET profile_picture = ? WHERE user_id = ?";
                    $stmt = mysqli_prepare($connection, $query);
                    mysqli_stmt_bind_param($stmt, "si", $fileName, $user_id);
                    mysqli_stmt_execute($stmt);
                } else {
                    // Insert profile picture with new profile row
                    $query = "INSERT INTO user_profiles (user_id, profile_picture) VALUES (?, ?)";
                    $stmt = mysqli_prepare($connection, $query);
                    mysqli_stmt_bind_param($stmt, "is", $user_id, $fileName);
                    mysqli_stmt_execute($stmt);
                }
